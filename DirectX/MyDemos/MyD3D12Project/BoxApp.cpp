	...
	for (; obj != objEnd; obj++)
	{
		...
		for (UINT submesh = 0; submesh < _RenderItem->SubmeshCount; submesh++)
		{
			...
			else if (_RenderData->mFormat == "PMX")
			{
				...
				if (!mWeightImage)
					throw std::runtime_error("Weight Image�� �ε��ϴµ� ����");
...
)
{
	GeometryGenerator Geom;
	{
		...
		if (r->_Instance.size() == 0)
			throw std::runtime_error("Instance Size�� 0�Դϴ�.");
...
)
{
	...
	assert(!res && "�ִϸ��̼� ������ �������� �ʴ� ������Ʈ�� CreateFBXObject��");
