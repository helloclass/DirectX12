...
Microsoft::WRL::ComPtr<ID3D12Resource> d3dUtil::storeDefaultBuffer(
	ID3D12Device* device,
	ID3D12GraphicsCommandList* cmdList,
	const void* initData,
	UINT64 byteSize,
...
std::string d3dUtil::getDDSFormat(std::string Path) {
	if (Path == "")
		return "";
...
std::wstring d3dUtil::String2Wstring(std::string str)
{
	std::wstring res;
...
DirectX::XMMATRIX d3dUtil::toMatrix(DirectX::XMFLOAT4X4 mat)
{
	DirectX::XMMATRIX res = DirectX::XMLoadFloat4x4(&mat);
	return res;
}

DirectX::XMFLOAT4X4 d3dUtil::toFloat4X4(DirectX::XMMATRIX mat)
{
